        ...
        <tbody>
          {currentItems.map((item, index) => (
            <tr key={index} className="hover:bg-[#D4EAFF] border border-b-1 bg-gray-100">
              <td className="text-center px-4 py-8 font-medium text-[14px] text-[#101828]">{item.monthYear}</td>
              <td className="text-center px-4 py-8 font-medium text-[14px] text-[#101828]">{item.high}</td>
              <td className="text-center px-4 py-8 font-medium text-[14px] text-[#101828]">{item.low}</td>
              <td className="text-center px-4 py-8 font-medium text-[14px] text-[#101828]">{item.hLPercentage}</td>
              <td
                className={`text-center px-4 py-8 font-medium text-[14px] leading-[20px] ${
                  item.movePercentage < 0 ? 'bg-[#FF3636] bg-opacity-50' : 'bg-[#26A641] bg-opacity-50'
                }`}
              >
                {item.movePercentage}%
              </td>
